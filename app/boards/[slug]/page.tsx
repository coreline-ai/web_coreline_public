...
export default function BoardDetailPage() {
    ...
    const fetchBoardData = useCallback(async () => {
        ...
        try {
            ...
            if (err?.message?.includes('401') || err?.status === 401 || err?.message?.includes('Login required')) {
                router.replace(`/login?error=SessionExpired&callbackUrl=/boards/${slug}`);
                return;
            }
    ...
    // Static Metadata for immediate rendering
    const STATIC_BOARD_METADATA: Record<string, { name: string; description: string }> = {
        'CL_Project_QnA': {
            name: '프로젝트 기술지원 QnA',
            description: '코어라인 프로젝트 진행 관련 문답 고객 게시판'
        },
        'news': {
            name: 'AI 뉴스',
    ...
    const staticInfo = STATIC_BOARD_METADATA[slug as string];
    const displayTitle = data?.board?.name || staticInfo?.name;
    const displayDesc = data?.board?.description || staticInfo?.description;

    // Skeleton Loader Component (Content Only)
    const BoardContentSkeleton = () => (
        <div className="animate-pulse">
            {/* Toolbar Skeleton */}
            <div className="mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex gap-2">
                    <div className="h-10 w-20 rounded-xl bg-gray-200 dark:bg-gray-800"></div>
                    <div className="h-10 w-24 rounded-xl bg-gray-200 dark:bg-gray-800"></div>
                    <div className="h-10 w-24 rounded-xl bg-gray-200 dark:bg-gray-800"></div>
                </div>
                <div className="h-10 w-32 rounded-xl bg-gray-200 dark:bg-gray-800"></div>
            </div>

            {/* Content Skeleton */}
            <div className="grid gap-4">
                {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="h-40 rounded-xl border-2 border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900"></div>
                ))}
            </div>
        </div>
    );

    // Title Skeleton (fallback if no static info)
    const TitleSkeleton = () => (
        <div className="mb-12 border-l-[6px] border-gray-200 pl-6 dark:border-gray-800 animate-pulse">
            <div className="mb-4 h-10 w-64 rounded bg-gray-200 dark:bg-gray-800"></div>
            <div className="h-6 w-96 rounded bg-gray-200 dark:bg-gray-800"></div>
        </div>
    );
        ...
        <div className="min-h-screen bg-white font-sans text-black dark:bg-[#111] dark:text-white bw:bg-white bw:text-black">
            ...
            <main className="relative pt-32 pb-24">
                <div className="mx-auto max-w-[1000px] px-4">
                    {/* Header: Render Immediately if we have data OR static info */}
                    {displayTitle ? (
                        <div className="mb-12 border-l-[6px] border-[#FFD600] pl-6">
                            <h1 className="mb-4 text-4xl font-black tracking-tight">{displayTitle}</h1>
                    ...
                    {isLoading ? (
                        ...
                        <div className="p-20 text-center font-black text-red-500">{error}</div>
                    ) : data ? (
                        <>
                            {/* Toolbar */}
                            <div className="mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
