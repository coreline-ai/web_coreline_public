  ...
  return (
    <>
      <nav className="pointer-events-none fixed top-0 right-0 left-0 z-50 flex justify-center px-4 py-6">
        <div className="pointer-events-auto flex w-full max-w-[1200px] items-center justify-between rounded-xl border-2 border-black bg-white px-6 py-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all duration-300 dark:border dark:border-white/20 dark:bg-black/80 dark:shadow-none dark:backdrop-blur-md">
          <div
            className="flex cursor-pointer items-center gap-3"
            onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          >
            <div className="relative h-10 w-10 overflow-hidden rounded-lg border-2 border-black transition-all duration-300 dark:border-transparent">
          ...
          <div className="hidden items-center gap-6 md:flex">
            <div className="flex items-center gap-8 font-bold text-black transition-colors duration-300 dark:text-gray-300">
              {['서비스', '포트폴리오', '프로세스', '소개'].map((item, i) => (
                <a
                  key={i}
                  href={`${item === '포트폴리오' ? '#projects' : item === '서비스' ? '#services' : item === '프로세스' ? '#process' : '#about'}`}
                  className="decoration-black decoration-2 underline-offset-4 transition-all hover:text-black hover:underline dark:decoration-[#FFD600] dark:hover:text-white"
                >
                  {item}
                </a>
              ))}
            ...
            <div className="mx-2 h-6 w-px bg-gray-300 dark:bg-gray-700"></div>
          ...
          <div className="flex flex-col gap-6 text-2xl font-black">
            {['서비스', '포트폴리오', '프로세스', '소개'].map((item, i) => (
              <a
                ...
                href={`${item === '포트폴리오' ? '#projects' : item === '서비스' ? '#services' : item === '프로세스' ? '#process' : '#about'}`}
                onClick={() => setIsMobileMenuOpen(false)}
                className="border-b-2 border-black pb-4 transition-all hover:pl-4 dark:border-white/10 dark:text-gray-300 dark:hover:text-white"
              >
                {item}
              </a>
            ))}
